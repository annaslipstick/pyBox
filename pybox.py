...
class VMConsole(tk.Toplevel):
    def __init__(self, master, vm_name, proc, thread):
...
def create_advanced_os(manager):
    ...
    main_path = os.path.join(iso_dir, "main.py")
    manager.add_vm(name, main_path, description, ram, cpu)
    messagebox.showinfo("OS Created", f"{name} OS created with full skeleton.")
...
class PyBoxGUI(tk.Tk):
    def __init__(self, manager):
        ...
        buttons = [
            ...
            ("New Advanced OS", lambda: create_advanced_os(self.manager)),
            ("Refresh", self.refresh_vm_list),  # <-- Refresh button added here
        ]
...
# The create_advanced_os function can be either outside or inside this file,
# just make sure it's imported or defined where PyBoxGUI can call it.
